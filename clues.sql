...
-- Write SQL query here
SELECT code, name FROM countries c 
JOIN countrylanguages cl ON code = cl.countrycode 
WHERE cl.language = 'Italian' AND code != 'VAT'
AND code IN (
    SELECT countrycode 
...
-- Write SQL query here
SELECT city.name AS city, contry.name AS country FROM cities city
JOIN countries contry ON city.countrycode = contry.code
WHERE city.name LIKE 'Serr%' AND contry.continent = 'South America';
...
-- Write SQL query here
SELECT city.name AS capital FROM countries country
JOIN cities city ON country.capital = city.id
